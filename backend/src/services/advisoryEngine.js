...
export function generateAdvisory(weather, crop) {
  ...
  for (const rule of advisoryRules) {
    const ruleCrop = rule.crop === "ALL" ? "ALL" : rule.crop.trim().toLowerCase();

    const appliesToCrop =
      ruleCrop === "ALL" || ruleCrop === normalizedCrop;
  ...
  if (advice.length === 0) {
    advice.push("Weather conditions appear suitable for normal farming activities.");
  }
